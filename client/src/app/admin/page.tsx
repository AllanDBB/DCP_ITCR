...
import { Loader2, Upload, Database, BarChart3, Trash2, Edit, Eye } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<'upload' | 'datasets' | 'stats'>('upload');
  ...
  // Cargar datasets y estadísticas
  useEffect(() => {
    if (isAuthenticated && user && ['admin', 'superadmin'].includes(user.role || 'user')) {
      loadDatasets();
      loadStats();
    }
